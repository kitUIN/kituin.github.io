...
|`res.raw[0].anilistID`       |匹配的[Anilist  ID](https://anilist.co/) (输出结果与 `anilistInfo` 值有关)|int/TraceMoeAnilist|
...
::: warning 注意事项
若在使用 `search()` 方法时将 `anilistInfo` 设为 `True` (默认),值 `anilistID` 类型为`TraceMoeAnilist`(见下表)，反之则为 `int`  
|变量              |   内容             |  类型  |
|----              | ----              | ----  |
|`res.raw[0].anilistID.id`       |匹配的[Anilist  ID](https://anilist.co/) |int|
|`res.raw[0].title`            |番剧名字|str|
|`res.raw[0].idMal`           |匹配的[MyAnimelist  ID](https://myanimelist.net/) |int|
|`res.raw[0].synonyms`         |备用英文标题|list|
|`res.raw[0].isAdult`         |是否R18|bool|
:::
